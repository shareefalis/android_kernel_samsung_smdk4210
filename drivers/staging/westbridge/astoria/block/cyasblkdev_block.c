...
static struct cyasblkdev_blk_data *gl_bd = NULL ;

static int g_is_block_dev_ready = 0;
	...
	if (bd) {
		...
		for (bus_num = 0; bus_num <= 1; bus_num++) {
			...
			for(i=0;i<9;i++)
			{
				...
				else if (stat == CY_AS_ERROR_TIMEOUT) {
				#ifndef WESTBRIDGE_NDEBUG
				cy_as_hal_print_message(
					"%s: time out on %d device "
						"- looping %d \n",
					__func__, bus_num,i) ;
				#endif
				} else {
