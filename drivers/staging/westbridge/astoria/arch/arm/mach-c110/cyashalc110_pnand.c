...
static int cy_as_hal_configure_interrupts(void *dev_p)
{
	...
	#endif
	irq = gpio_to_irq(WB_CYAS_INT);
	irq_set_irq_type(irq, IRQ_TYPE_EDGE_FALLING);
	result = request_irq(irq,	(irq_handler_t)cy_astoria_int_handler, IRQF_SHARED, "AST_INT#", dev_p);
#else
	...
	if (result == 0) {
		...
		gpio_free(irq_pin);
		cy_as_hal_print_message(KERN_WARNING"ASTORIA: can't get assigned IRQ%i for INT#%d \n", irq, irq_pin);
		have_irq = false;
	}
...
#ifdef DBGPRN_ENABLED
			DBGPRN("<1>cy_service_e_p_dma_read_request cb: xfer_sz:%d ep:%d \n",
				size,ep);
#endif
	/*
...
int cy_as_hal_configure_sd_isr(void *dev_p, irq_handler_t isr_function)
{
	...
	result = request_irq(irq, (irq_handler_t)isr_function, IRQF_SHARED, "AST_CD#", dev_p);
